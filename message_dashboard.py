...
#######################
# OpenAI setup for topic classification
    ...
    if st.session_state["logged_in"]:
        ...
        if st.button("Login"):
            if username == USERNAME and password == PASSWORD:
                ...
                # Set query params using the new method
                st.session_state.query_params = st.query_params
                st.session_state.query_params.update({'logged_in': 'true'})
                
                # Redirect to dashboard without rerun
                st.write("Inicio de sesión exitoso!")
                time.sleep(1)
                st.rerun()  
                # You can use session state or update the UI without rerunning
...
# Display the DataFrame with the correct column mapping
    ...
    "session_id": st.column_config.TextColumn("Session ID"),  # Add session_id column for display
    })
...
# Iterate over the sorted DataFrame
    ...
    unsafe_allow_html=True
     ...
     # Truncate long messages and show a 'Show more' option
